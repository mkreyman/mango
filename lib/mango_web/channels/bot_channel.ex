defmodule MangoWeb.BotChannel do
  ...
  alias Mango.{Sales, Catalog.Product, Repo}
  ...
  def handle_in("add", payload, socket) do
    ...
    reply =
      case [socket.assigns[:order], item] do
        [nil, _] ->
          %{message: "Please create a new order before proceeding."}

        [order, [product_id, quantity]] ->
          {:ok, order} =
            Sales.add_to_cart(order, %{"product_id" => product_id, "quantity" => quantity})

          socket = socket |> assign(:order, order)
          product = Repo.get(Product, product_id)
          %{message: "#{quantity} of #{product.name} product added to order."}
